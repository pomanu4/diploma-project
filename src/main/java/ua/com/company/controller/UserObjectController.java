...
import ua.com.company.validator.UserValidator;
...
public class UserObjectController {
	...
	public String adminSetNewEmail(@RequestParam("userId") int user_id, Model model,
			@RequestParam("newImail") String email) {
		userService.setNewEmail(email, user_id);
		User user = userService.oneUserWithInfo(user_id);
		model.addAttribute("user", user);
		return "adminUserDetail";
	}
	...
	public String adminSetNewPassword(@RequestParam("userId") int user_id, Model model,
	...
	public String adminSetNewRole(@RequestParam("userId") int user_id, Model model,
	...
	public String adminBlockUserAccount(@RequestParam("userId") int user_id, Model model,
			@RequestParam("status") boolean value) {
	...
	public String registrationProcess(@ModelAttribute("user") @Valid User user, BindingResult result,
			@RequestParam("userPhoto") MultipartFile photo,
			@RequestParam("contactInformation") String contactInfo,	Model model) {
		if (result.hasErrors()) {
			return "registration";
		}
