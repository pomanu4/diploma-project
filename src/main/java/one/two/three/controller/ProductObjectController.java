...
import one.two.three.components.ProductListWrapper;
...
public class ProductObjectController {
	...
	public String addMoreProduct(@RequestParam("prodid") int product_id, @RequestParam("prodQantity") int howManyAdd, Model model) {
	...
	public String addNewInfo(@RequestParam("prodid") int product_id, @RequestParam("prodInfo") String newInfo, Model model) {
	...
	public String prodByID(Model model, @PathVariable("id") int id) {
		...
		Product productComm = productService.oneProdWithCommentandComUser(id);
		Product productWithOwner = productService.productWithOwnerById(id);
...
///////////	
	...
	public String productSortedByName(Model model, @SessionAttribute("prodListContainer") ProductListWrapper wrap) {
		
	List<Product> productList = productService.sortByName(wrap.getPl());
	ProductListWrapper sortWrap = new ProductListWrapper();
	sortWrap.setPl(productList);
	model.addAttribute("prodListContainer", sortWrap);
	...
	List<Product> productList = productService.sortByPrice(wrap.getPl());
	ProductListWrapper sortWrap = new ProductListWrapper();
	sortWrap.setPl(productList);
	model.addAttribute("prodListContainer", sortWrap);
	...
	public String addProduct(@ModelAttribute("product") @Valid Product prod,
		...
		if(result.hasErrors()) {
			model.addAttribute("text", "erroor");
			return "userpage";
		}
	...
	public String addPicture(@RequestParam("picture") MultipartFile picture, @RequestParam("prodId") int id, Model model) {
